 ...
 * MSTART -> decorator
 * ......
 * MSTART -> encoding_decl
 * MSTART -> yield_expr
 * 
 * total: 85 rules which mapping to 85 DFAs
 * 
 */
static nfagrammar *
metacompile(node *n)
 ...
 * RULE -> NAME : RHS
 * 
 * For RULE single_input: 
 * single_input : NEWLINE | simple_stmt | compound_stmt NEWLINE
 *     ^        ^                   ^
 *     |        |                   |
 *    NAME    COLON                RHS
 * RHS -> NEWLINE | simple_stmt | compound_stmt NEWLINE
 *
 * again for RULE file_input:
 * file_input : (NEWLINE | stmt)* ENDMARKER
 *     ^      ^             ^
 *     |      |             |
 *    NAME  COLON          RHS
 * RHS -> (NEWLINE | stmt)* ENDMARKER
 *
 */
static void
 ...
 *          ^     ^       ^     ^         ^
            |     |       |     |         |
 *         ALT1  VBAR    ALT2  VBAR      ALT3
 * ALT1 -> NEWLINE
 * ALT2 -> simple_stmt
 * ALT3 -> compound_stmt NEWLINE
 *
 * again for RULE file_input:
 * file_input : (NEWLINE | stmt)* ENDMARKER
 *     ^      ^             ^
 ...
 *                                                                 |
 *                                                                 V
 * compound_stmt is just another RULE, which will be compiled in MSTART -> compound_stmt,
